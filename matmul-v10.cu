...
// Running cost of CUDA kernel is 1.06895ms
// TFLOPS: 110.744
...
#pragma unroll
  for (int i = 0; i < 8; ++i) {
    // for (int j = 0; j < 2; ++j) {
    for (int k = 0; k < 2; ++k) {
      int row = tz * 64 + i / 2 * 16 + i % 2 * 8 + tx / 4;
...
#pragma unroll
  for (int i = 0; i < 8; ++i) {
    // for (int j = 0; j < 2; ++j) {
    for (int k = 0; k < 2; ++k) {
      int row = ty * 32 + i / 2 * 16 + i % 2 * 8 + tx / 4;
      int col = ki * KII + k * 8 + tx % 4 * 2;
      col = row % 2 * 32 + col;
      row = row / 2;
      col = col ^ ((row & 3) << 3);
...
__device__ __inline__ void mmaSync(unsigned int *fragA, unsigned int *fragB,
                                   float *accum) {
