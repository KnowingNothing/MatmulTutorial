...
class HyperCube:
    def __init__(
        self, ndim: int, dimensions: Optional[List[Union[int, IntegerType]]] = None
    ) -> None:
        """
        dimensions:
        ...
        self.ndim = ndim
        self.dimensions = (
            ([IntegerType(unsigned=False, bits=32)] for _ in range(ndim))
        ...
        assert all(
            [self.dim_is_dynamic(i) or self.dimensions[i] > 0 for i in range(self.ndim)]
    ...
    def __setitem__(self, key: int, value: Union[int, IntegerType]):
        assert key >= 0 and key < self.ndim
        assert (
            isinstance(value, IntegerType) or value > 0
        ), "Static dim value should be > 0."
    ...
    def get_static_dims_with_keys(self):
        return [
            (i, self.dimensions[i]) for i in range(self.ndim) if self.dim_is_static(i)
        ]

    def num_elements(self):
        assert (
            not self.has_dynamic()
        ), "Can't get number of elements for dynamic HyperCube."
