...
class IntegerType(DType):
    ...
    def is_same(self, another_type: "IntegerType"):
        return (
            isinstance(another_type, IntegerType)
            and (self.unsigned == another_type.unsigned)
            and (self.bits == another_type.bits)
        )
...
class FloatingType(DType):
    ...
    def _is(self, sign: int, exp: int, mantissa: int):
        return (
            self.sign_bits == sign
            and self.exp_bits == exp
            and self.mantissa_bits == mantissa
        )
    ...
    def is_same(self, another_type: "FloatingType"):
        return (
            isinstance(another_type, FloatingType)
            and (self.sign_bits == another_type.sign_bits)
            and (self.exp_bits == another_type.exp_bits)
            and (self.mantissa_bits == another_type.mantissa_bits)
        )
