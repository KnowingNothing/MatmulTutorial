// 2 mma + pipeline + smem crosswise + partial ldmatrix + no unroll + serpentine + ptx opt
...
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    ...
    asm volatile(
      ...
      "r"(smem_ptr), "l"(&A[(by * 128 + logic_row) * K + (ko * KI + logic_col)]), "n"(16), "r"(16)
    );
...
__device__ __forceinline__ void predLoadSmemA(half *smem, half *A, int M, int K,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    "}\n" :: "r"((int)pred_guard),
    "r"(smem_ptr), "l"(&A[(by * 128 + logic_row) * K + (ko * KI + logic_col)]), "n"(16)
    );
  }
}
...
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    ...
    asm volatile(
      "cp.async.cg.shared.global.L2::128B [%0], [%1], %2, %3;\n" ::
      "r"(smem_ptr), "l"(&B[(bx * 128 + logic_row) * K + (ko * KI + logic_col)]), "n"(16), "r"(16)
    );
  }
}
...
__device__ __forceinline__ void predLoadSmemB(half *smem, half *B, int N, int K,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    " @p cp.async.cg.shared.global.L2::128B [%1], [%2], %3;\n"
    "}\n" :: "r"((int)pred_guard),
    "r"(smem_ptr), "l"(&B[(bx * 128 + logic_row) * K + (ko * KI + logic_col)]), "n"(16)
    );
  }
}
