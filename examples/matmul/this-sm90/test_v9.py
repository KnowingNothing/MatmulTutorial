#!/usr/bin/env python3
"""
Level 9: Fully Async Warpgroup Scheduling
"""
...
def compile():
    import time
    src = os.path.join(os.path.dirname(__file__), "matmul-v9.cu")
    out = f"/tmp/gemm_bf16_level9_{int(time.time())}.so"
    r = subprocess.run([
...
if __name__ == "__main__":
    print("Level 10: Fully Async Warpgroup Scheduling")
    print(f"BM={BM}, BN={BN}, BK={BK}, Cluster={CLUSTER_SIZE}x1x1")
    print("Improvement: Independent TMA/Math tile schedulers, no __syncthreads()\n")
