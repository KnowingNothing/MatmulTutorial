#!/usr/bin/env python3
"""
Level 2: TMA + WGMMA GEMM Test

Key insight: TMA 128B swizzle == WGMMA B128 layout!
- TMA 128B swizzle: k' = k XOR ((m & 7) * 8)
- WGMMA B128 (layout_type=1): same formula
- No rearrangement needed - TMA data directly usable by WGMMA

Layout (K-major):
- A[M][K]: K is contiguous
- B stored as B^T[N][K]: K is contiguous

BLOCK_K = 64 to match TMA 128B atom (128 bytes / 2 bytes per bf16)
"""
import torch
import ctypes
import subprocess
...
# Block sizes - BLOCK_K=64 for TMA 128B atom
...
class TMAHelper:
    ...
    def create_k_major_desc(self, tensor, shape_k, shape_mn, outer_stride):
        """
        Create TMA descriptor for K-major matrix with 128B swizzle.
        
        For K-major layout (K contiguous):
        - gmem_inner = K (contiguous dimension)
        - gmem_outer = M or N
        - gmem_strides = outer_stride * elem_size
        - smem_inner = 64 (128 bytes / 2 bytes per bf16)
        - smem_outer = BLOCK_M or BLOCK_N
        """
        ...
        gmem_inner = shape_k
        gmem_outer = shape_mn
        smem_inner = 64  # 128B / 2 bytes = 64 bf16 elements
        smem_outer = BLOCK_M
...
def compile_kernel():
    ...
    kernel_path = os.path.join(kernel_dir, "matmul-v2.cu")
    so_path = "/tmp/gemm_bf16_level2.so"
...
def test_correctness(lib, M=1024, N=1024, K=1024):
    print("\n=== Correctness Test: {}x{}x{} ===".format(M, N, K))
    
    tma = TMAHelper()
    
    # A[M][K] row-major: K is contiguous
    A = torch.randn(M, K, dtype=torch.bfloat16, device='cuda')
    # B[K][N] row-major, store as B^T[N][K] for K-contiguous access
    B = torch.randn(K, N, dtype=torch.bfloat16, device='cuda')
    B_t = B.t().contiguous()  # [N][K], K contiguous
    
    C = torch.zeros(M, N, dtype=torch.float32, device='cuda')
    
    # TMA descriptors with 128B swizzle
...
def main():
    print("Level 3: TMA + WGMMA GEMM")
    print("=" * 50)
    print("BLOCK_M={}, BLOCK_N={}, BLOCK_K={}".format(BLOCK_M, BLOCK_N, BLOCK_K))
    print("TMA 128B swizzle == WGMMA B128 layout (no rearrangement)")
