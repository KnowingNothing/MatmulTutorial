#!/usr/bin/env python3
"""
Level 7: L2 Cache Swizzle

Improvement over Level 6:
- L2 cache-friendly tile scheduling using swizzle pattern
- Reorders tile processing so adjacent SMs share L2 cache data
"""
...
def compile():
    src = os.path.join(os.path.dirname(__file__), "matmul-v7.cu")
    out = "/tmp/gemm_bf16_level7.so"
    r = subprocess.run([
...
def create_desc_a(tensor, inner, outer, stride):
    ...
    box = (ctypes.c_uint32 * 2)(BK, BM)  # Load 64x128 in one TMA
...
def create_desc_b(tensor, inner, outer, stride):
    ...
    dims = (ctypes.c_uint64 * 2)(inner, outer)
    strides = (ctypes.c_uint64 * 1)(stride * 2)
    box = (ctypes.c_uint32 * 2)(BK, BN // 2)  # 64 x 128 (each CTA loads half)
    elem = (ctypes.c_uint32 * 2)(1, 1)
    libcuda.cuTensorMapEncodeTiled(ctypes.byref(tmap), 9, 2,
        ctypes.c_void_p(tensor.data_ptr()), dims, strides, box, elem, 0, 3, 3, 0)
    return tmap
...
if __name__ == "__main__":
    print("Level 8: L2 Cache Swizzle")
    print(f"BM={BM}, BN={BN}, BK={BK}, Cluster={CLUSTER_SIZE}x1x1")
    print("Improvement: L2 cache-friendly tile scheduling")
    ...
    if all(test(lib, s) for s in [512, 1024, 2048, 4096]):
        ...
        bench(lib, 4096)
        bench(lib, 8192)
        bench(lib, 12288)  # L2 swizzle shines on larger sizes
