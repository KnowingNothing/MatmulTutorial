/**
 * Level 6: Persistent Kernel + m64n256k16 + Cluster Multicast
 * 
 * Features:
 * - Persistent kernel: each cluster loops over multiple tiles
 * - m64n256k16 WGMMA for larger N dimension
 * - 3 warpgroups: 1 TMA + 2 math (each handles 64 M rows)
 * - Cluster multicast for B matrix
 * 
 * BM=128, BN=256, BK=64 per CTA
 * Cluster 2x1x1: 2 CTAs share B via TMA multicast
 */
 ...
 __device__ void get_coord_n256(int ltid, int r, int& row, int& col) {
     int chunk = r / 32;
     int local_reg = r % 32;
 ...
 void kernel(const __grid_constant__ TmaDescriptor dA, const __grid_constant__ TmaDescriptor dB,
     ...
     // Register reconfiguration: TMA warpgroup uses fewer, math warpgroups use more
     if (wg == 0) {
     ...
     // ===== Persistent Loop: stride by num_clusters =====
     for (int tile_idx = cluster_id; tile_idx < num_tiles; tile_idx += num_clusters) {
         ...
         // Convert tile index to coordinates
         ...
         if (wg == 0 && tid == 0) {
             for (int k = 0; k < nk; k++) {
                 ...
                 if (cta == 0) {
                     tma_multicast_2d(&dB, full[s], sB[s], k * BK, bn, 0x3);
                 }
                 full[s]->arrive_tx(TX);
             }
         }
