// level7/matmul.cu
// SM100 (GB200) BF16 GEMM — Dual-Tile 2SM Architecture
//
// THE critical optimization in this level:
//   In cta_group::2 UMMA, each SM reads A from its OWN CTA's shared memory.
//   By assigning DIFFERENT tiles to the two CTAs in a cluster (different m_blocks,
//   same n_block), a single UMMA instruction processes TWO tiles simultaneously:
//     - SM0 computes: A_CTA0 × B_combined → SM0 TMEM (tile 0)
//     - SM1 computes: A_CTA1 × B_combined → SM1 TMEM (tile 1)
//   B data is shared across both CTAs (each loads 64 of 128 columns).
//   This doubles effective compute utilization vs Level 6 where both SMs
//   computed the same tile.
//
// Architecture:
//   - Scheduler assigns tiles per-CTA (blockIdx.x), not per-cluster
//   - 2D swizzle ensures paired CTAs (2k, 2k+1) share the same n_block
//   - Both CTAs run epilogue independently (each reads its own SM's TMEM)
//   - tmem_empty barrier uses cluster-addressed arrive from CTA1
//
// Inherited optimizations:
//   - BLOCK_M=256, BLOCK_N=128, BLOCK_K=64, 4-stage pipeline
...
//
// Warp roles:
//   Warp 0:    TMA load  (1 elected thread per CTA)
//   Warp 1:    MMA issue (1 elected thread, leader CTA only)
//   Warp 2:    TMEM alloc/dealloc
//   Warp 3:    idle
//   Warps 4-7: Epilogue  (128 threads, BOTH CTAs)
//
// Computes D = A @ B^T
...
// M-wave: each UMMA covers WAVE_BLOCK_M=128 rows per SM
...
// Arrive at a barrier on a REMOTE CTA in the cluster (for tmem_empty signaling)
...
// ============================================================
//  2D Swizzle Tile Scheduler — PER-CTA (not per-cluster)
//
//  Each CTA uses blockIdx.x as its ID and num_sms as the stride.
//  The swizzle groups M-blocks in groups of SWIZZLE_GROUP_SIZE=8,
//  ensuring that CTAs (2k, 2k+1) in the same cluster always get
//  the same n_block (so they share B data) but different m_blocks.
// ============================================================
struct TileScheduler {
    ...
    __device__ TileScheduler(uint32_t M, uint32_t N, uint32_t nc)
        : num_m_blocks((M + BLOCK_M - 1) / BLOCK_M),
          ...
          num_ctas(nc),
          cta_id(blockIdx.x),  // ← KEY CHANGE: per-CTA, not per-cluster
          current_iter(-1) {}
    __device__ bool get_next_block(uint32_t& m_block, uint32_t& n_block) {
        ...
        // 2D swizzle: group M-blocks, then iterate N
...
bf16_gemm_2sm_kernel(
    ...
    uint32_t num_ctas)      // ← now num_ctas (= num_sms), not num_clusters
{
    ...
    if (warp_idx == 1 && elect_one()) {
        for (uint32_t s = 0; s < NUM_STAGES; ++s) {
            barrier_init(full_bar[s],  CLUSTER_SIZE);  // both CTAs' TMA arrivals
            barrier_init(empty_bar[s], 1);             // UMMA multicast commit
        }
        for (uint32_t e = 0; e < NUM_EPILOGUE_STAGES; ++e) {
            barrier_init(tmem_full_bar[e],  1);            // UMMA multicast commit
            // ← KEY CHANGE: init = CLUSTER_SIZE (both CTAs must signal)
    ...
    if (warp_idx == 0 && elect_one()) {
        // ======================== TMA WARP ========================
        // Each CTA has its OWN scheduler → loads A for its OWN tile.
        // Both CTAs load B for the same n_block (different 64-col halves).
        ...
        while (scheduler.get_next_block(m_block, n_block)) {
            // ← KEY CHANGE: m_coord comes from THIS CTA's tile
        ...
        // ======================== MMA WARP ========================
        // Uses leader CTA's scheduler. The UMMA instruction implicitly
        // processes BOTH CTAs' tiles: SM0 reads A from CTA0's SMEM,
        // SM1 reads A from CTA1's SMEM. B is combined from both CTAs.
        TileScheduler scheduler(M, N, num_ctas);
        uint32_t m_block, n_block;
        uint32_t stage = 0, phase = 0;
        while (scheduler.get_next_block(m_block, n_block)) {
        ...
        // ======================== EPILOGUE WARPS ========================
        // ← KEY CHANGE: BOTH CTAs run epilogue (no is_leader gate).
        // Each CTA reads its own SM's TMEM and stores to its own tile's output.
        // CTA0's TMEM has tile 0's result, CTA1's TMEM has tile 1's result.
        TileScheduler scheduler(M, N, num_ctas);
        uint32_t m_block, n_block;
        ...
        while (scheduler.get_next_block(m_block, n_block)) {
            ...
            for (uint32_t w = 0; w < NUM_MWAVES; ++w) {
                ...
                // Signal tmem_empty on last wave — both CTAs must signal the LEADER's barrier
                if (w == NUM_MWAVES - 1) {
                    tcgen05_fence_before();
                    named_barrier_sync(NUM_EPILOGUE_THREADS, EPILOGUE_BAR_ID);
                    // ← KEY CHANGE: cluster-addressed arrive for CTA1
                    if (local_tid == 0) {
                        if (is_leader)
                ...
                // TMA Store — each CTA stores to its OWN tile's coordinates
                tma_store_fence();
                if (local_tid == 0) {
...
static void launch_kernel(
    ...
    // ← KEY CHANGE: grid = num_sms CTAs (not num_clusters)
    // Each CTA gets its own tile. num_sms must be even for cluster_size=2.
    uint32_t num_ctas = min((uint32_t)num_sms, num_tiles);
    // Ensure even number of CTAs for cluster_size=2
