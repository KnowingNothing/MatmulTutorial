// =============================================================================
// FP8 GEMM Level 2 — SM100 (GB200)
//
// D (BF16, M×N) = A (FP8 E4M3, M×K) × B^T (FP8 E4M3, N×K)
//
// Level 2 improvements over Level 1:
//   1. Removed debug printf from kernel hot path
//   2. Dynamic BLOCK_N selection (compile-time template) for better SM utilization
//   3. SMEM descriptor precomputation per stage using warp-lane shuffle
//   4. Maximized pipeline stages (computed from SMEM budget at compile time)
//   5. Shape-aware tile scheduler with improved L2 locality
//
...
//
// Pipeline: N_STAGES stages with 3 barrier sets
// =============================================================================
...
// =============================================================================
// Configuration — templated on BLOCK_N for dynamic tile selection
// =============================================================================

// These are set at compile time via -D flags:
//   TILE_N = 128 | 160 | 192 | 224 (default 128)
#ifndef TILE_N
#define TILE_N 128
#endif
...
static constexpr uint32_t BLOCK_K = 128;   // FP8 uses 128
static constexpr uint32_t UMMA_K  = 32;    // FP8 uses 32
...
static constexpr uint32_t SMEM_A   = BLOCK_M * BLOCK_K * 1;            // FP8=1B
...
// Swizzle mode for CD: pick the largest mode where (BLOCK_N * 2) % mode == 0
...
// Maximize pipeline stages within 232448 bytes SMEM
static constexpr uint32_t SMEM_CAPACITY = 232448;
static constexpr uint32_t SMEM_FIXED = SMEM_CD + 4;  // CD + tmem_ptr
// Barriers per stage: full + empty + sf_full = 3 × 8 = 24 bytes
// Epilogue barriers: tmem_full + tmem_empty = 2 × NUM_EPI_STAGES × 8 = 32 bytes
static constexpr uint32_t SMEM_EPI_BAR = NUM_EPI_STAGES * 2 * 8;
static constexpr uint32_t SMEM_PER_STAGE = SMEM_STAGE + 3 * 8;  // data + 3 barriers

static constexpr uint32_t NUM_STAGES =
    (SMEM_CAPACITY - SMEM_FIXED - SMEM_EPI_BAR) / SMEM_PER_STAGE;
// Cap at 32 (warp-lane based precomputation limit)
...
// SMEM descriptor for A/B data (SWIZZLE_128B)
...
//     [46:49)  version = 1 (SM100)
//     [49:61)  reserved
//     [61:64)  layout_type: 0=NONE, 1=SWIZZLE_32B, 2=SWIZZLE_64B, 3=SWIZZLE_128B
//
// For K-major FP8 with SWIZZLE_128B:
//   SBO = 8 × BLOCK_K × sizeof(FP8) = 8 × 128 × 1 = 1024
//   LBO = 0 (only one atom on K axis)
__device__ __forceinline__ uint64_t make_smem_desc(void* ptr, uint32_t sbo, uint32_t layout = 2) {
    // layout=2 for SWIZZLE_128B (CuTe enum: NONE=0, 128B_BASE32B=1, 128B=2, 64B=4, 32B=6)
    uint64_t d = 0;
    ...
    d |= (uint64_t)1 << 46;                      // version = 1 [46:49)
    d |= (uint64_t)layout << 61;                  // layout_type [61:64)
    return d;
}

// SMEM descriptor for scale factors (SWIZZLE_NONE)
//   SBO = 8 × 16 = 128 (UTCCP atom stride in MN direction)
//   LBO = 0
__device__ __forceinline__ uint64_t make_sf_desc(void* ptr) {
    ...
    d |= (uint64_t)1 << 46;
    // layout_type = 0 (SWIZZLE_NONE) — default
    return d;
}

// Replace just the start_address field [0:14) in an existing descriptor
__device__ __forceinline__ void replace_desc_addr(uint64_t& desc, void* ptr) {
    uint32_t addr = (uint32_t)__cvta_generic_to_shared(ptr) >> 4;
    // Clear old address and set new
    ...
    uint32_t M, uint32_t N, uint32_t K)
{
    ...
    // Warp 1: MMA Issue
    //
    // Level 2 optimization: pre-compute per-stage descriptor lo values
    // in warp lanes and use __shfl_sync to select at runtime.
    // =====================================================================
    else if (wid == 1) {
        ...
        // Pre-compute per-stage SMEM descriptor base lo values
        // Each lane stores the lo value for its corresponding stage
        ...
        // Lane i stores the lo for stage i
        ...
        while (sched.next(mb, nb)) {
            ...
            for (uint32_t kb = 0; kb < num_kb; kb++) {
                ...
                // Fetch pre-computed stage descriptor lo from the right lane
