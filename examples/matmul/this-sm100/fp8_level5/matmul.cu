// =============================================================================
// FP8 GEMM Level 5 — SM100 (GB200) — Dynamic Tile + 2-CTA Multicast
//
// D (BF16, M×N) = A (FP8 E4M3, M×K) × B^T (FP8 E4M3, N×K)
//
// Level 5 improvements over Level 4:
//   1. Dynamic BLOCK_N selection at compile time (via -DTILE_N=64/128/224)
//      → Host-side heuristic picks the best BLOCK_N per problem size
//      → Minimizes wave count, then maximizes last-wave SM utilization
//   2. Properly handles all BLOCK_N variants:
//      - BLOCK_N=64:  SWIZZLE_CD=128B, CD_STORES=1, 9 stages
//      - BLOCK_N=128: SWIZZLE_CD=128B, CD_STORES=2, 7 stages
//      - BLOCK_N=224: SWIZZLE_CD=64B,  CD_STORES=7, 6 stages
//   3. Explicit full_bar wait in MMA warp (matches DeepGEMM's pattern)
//   4. Scheduler ensures paired CTAs always share the same N-block
//      (SWIZZLE_GROUP is always a multiple of NUM_MULTICAST)
//
...
static constexpr uint32_t SMEM_A   = BLOCK_M * BLOCK_K * 1;      // 16384
static constexpr uint32_t SMEM_B   = LOAD_BLOCK_N * BLOCK_K * 1;  // Half of single-CTA
...
static constexpr uint32_t STORE_N       = SWIZZLE_CD / 2;  // cols per TMA store atom
...
static constexpr uint32_t SMEM_FIXED = SMEM_CD + 4;   // CD staging + tmem_ptr
static constexpr uint32_t SMEM_EPI_BAR = NUM_EPI_STAGES * 2 * 8;  // tf_bar + te_bar
static constexpr uint32_t SMEM_PER_STAGE = SMEM_STAGE + 3 * 8;    // full + empty + sf_full
...
// cta_group::2 MMA: processes UMMA_M × UMMA_N tile across 2 CTAs
...
//   ptr:    pointer into shared memory (128B-aligned)
//   sbo:    stride byte offset = 8 × (leading_dim) × sizeof(elem)
//   layout: 2 = Swizzle128B, 1 = Swizzle64B, 0 = Swizzle32B
//
// Descriptor bit layout (64 bits):
//   [13:0]   = base_addr >> 4   (16-byte unit address within SMEM window)
//   [45:32]  = sbo >> 4         (stride in 16-byte units)
//   [46]     = 1                (valid bit)
//   [62:61]  = layout mode
...
// Layout mode 0 = non-swizzled, sbo = 128B (aligned to UTCCP requirement)
...
// For cta_group::2:
//   UMMA_M=256 → m_dim = 256/16 = 16 (bits [28:24])
...
struct Scheduler {
    ...
    __device__ bool next(uint32_t& m, uint32_t& n) {
        ...
        // Swizzle: cycle M-blocks within group, then advance N
        // This ensures consecutive CTAs (in a cluster) get same N-block
    ...
    uint32_t M, uint32_t N, uint32_t K)
{
    ...
    // TMEM allocation: cta_group::2 — both CTAs must call
    if (wid == 3)
                ...
                if (kb % NUM_SF_PER_LOAD == 0) {
                    ...
                    for (uint32_t i = 0; i < SF_BLOCK_N / 128; i++)
                        utccp_transpose(
                            ...
                            lane);
                    // Single fence covers all SMEM writes from both transposes
                    fence_async_smem();
                }
    ...
    // =====================================================================
        ...
        // SBO for SMEM descriptors: 8 rows × BLOCK_K bytes/row × 1 byte/elem
        ...
        while (sched.next(mb, nb)) {
            ...
            for (uint32_t kb = 0; kb < num_kb; kb++) {
                // Wait for TMA load + SF transpose (both CTAs)
                // sf_full_bar completion implies full_bar completion
                // (transposer waits on full_bar before signaling sf_full_bar)
                bar_wait(&sf_full_bar[stg], ph);
                tmem_fence_after();

                // UTCCP copy SF to TMEM (cta_group::2)
                ...
                // Get precomputed descriptor base for this pipeline stage
    ...
    // =====================================================================
        ...
        while (sched.next(mb, nb)) {
            ...
            for (uint32_t s = 0; s < CD_STORES; s++) {
                // Only wait for TMA store completion after pipeline is full
                if (s >= TMA_ST_STAGES) {
    ...
    int M, int N, int K)
{
    ...
    // B: FP8 K-major — box size uses LOAD_BLOCK_N
    CUtensorMap tma_b;
    {
