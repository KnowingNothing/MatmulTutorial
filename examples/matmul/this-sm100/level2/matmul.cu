// level2/matmul.cu
// SM100 (GB200) BF16 GEMM — 2-stage pipelined TMA+UMMA with cta_group::2 TMA
//
// Improvements over level1:
//   1. 2-stage SMEM double buffering with full/empty barriers per stage
//   2. cta_group::2 TMA: both CTAs' TMA loads signal leader's barrier,
//      eliminating cluster_sync() from the inner K loop
//   3. Only leader CTA writes output (eliminates redundant writes)
//   4. TMA swizzle 128B consistent with DeepGEMM
//
// Computes D = A @ B^T
...
// SMEM: [full_bar[0..1] empty_bar[0..1] tmem_bar tmem_addr | pad to 1024 | stages]
static constexpr uint32_t OFF_DATA  = 1024;
static constexpr uint32_t SMEM_SIZE = OFF_DATA + NUM_STAGES * SMEM_STAGE;

// Peer bit mask: clears bit 24 to route barrier/addr to leader CTA (rank 0)
...
//       per CUTLASS SM100_TMA_2SM_LOAD_2D implementation
    ...
    uint32_t num_n_blocks)
{
    ...
    // Layout: full[0..1](16B) empty[0..1](16B) tmem_bar(8B) tmem_addr(4B)
    ...
    if (warp_idx == 1 && elect_one()) {
        for (uint32_t s = 0; s < NUM_STAGES; ++s) {
            ...
            barrier_init(empty_bar[s], 1);   // 1 commit arrival
        }
    ...
    // ========== Pipeline: 2-stage, cta_group::2 TMA (no cluster_sync in K loop) ==========
    // Hoist warp role checks
    const bool is_tma_thread = (warp_idx == 0 && elect_one());
    const bool is_mma_thread = (is_leader && warp_idx == 1 && elect_one());

    uint32_t stage_idx = 0, phase = 0;

    for (uint32_t kb = 0; kb < num_k_blocks; ++kb) {
        // 1) Wait for SMEM[stage_idx] to be free
        //    Each CTA waits on its own empty_bar (multicast-signaled by UMMA commit)
        barrier_wait(empty_bar[stage_idx], phase ^ 1);

        // 2) Each CTA: arrive_expect_tx on leader's full_bar, then issue cta_group::2 TMA
        if (is_tma_thread) {
            // Arrive on leader's barrier with expected byte count BEFORE TMA
            if (is_leader) {
                barrier_arrive_expect_tx(full_bar[stage_idx], TMA_BYTES);
            } else {
                barrier_arrive_expect_tx_cluster(full_bar[stage_idx], TMA_BYTES, 0);
            }

            int32_t kc = kb * BLOCK_K;
            tma_load_2d_cg2(get_smem_a(stage_idx), &tma_a, full_bar[stage_idx], kc, m_coord);
            tma_load_2d_cg2(get_smem_b(stage_idx), &tma_b, full_bar[stage_idx], kc, n_coord);
        }

        // 3) Only leader waits for both CTAs' TMA to complete, then issues UMMA
        //    Non-leader CTA proceeds directly to the next iteration's empty_bar wait
        if (is_leader) {
            barrier_wait(full_bar[stage_idx], phase);
            ...
            if (is_mma_thread) {
                ...
                for (uint32_t k = 0; k < BLOCK_K / UMMA_K; ++k) {
                    uint64_t ad = ((uint64_t)ahi << 32) | (alo[stage_idx] + k * 2);
                    uint64_t bd = ((uint64_t)bhi << 32) | (blo[stage_idx] + k * 2);
                    uint32_t accum = (kb > 0 || k > 0) ? 1u : 0u;
                    umma_f16_cg2(0, ad, bd, idesc, accum);
                }

                // Multicast commit signals empty_bar on BOTH CTAs
                umma_commit_2sm(empty_bar[stage_idx]);
    ...
    // Only leader writes output (both CTAs compute same result)
    if (is_leader) {
