#!/usr/bin/env python3
"""Test script for Level 1 — Baseline 2SM UMMA kernel on SM100 (GB200).

Compiles the kernel, verifies correctness against torch.matmul,
and benchmarks performance using CUDA events.

Kernel computes: D = A @ B^T  (A: M×K, B: N×K, D: M×N, all BF16 row-major)
"""
import torch
import ctypes
import subprocess
...
SO_PATH = f"/tmp/gemm_bf16_sm100_{LEVEL}.so"
...
def compile_kernel():
    cmd = ["nvcc", "--shared", "-Xcompiler", "-fPIC",
           "-gencode", "arch=compute_100a,code=sm_100a", "-O2",
...
def benchmark(M=BENCH_SIZE, N=BENCH_SIZE, K=BENCH_SIZE,
    ...
    # torch.matmul reference perf (BF16)
    ...
    print(f"  Ratio: {torch_ms / ms:.3f}x (ours / torch)")
