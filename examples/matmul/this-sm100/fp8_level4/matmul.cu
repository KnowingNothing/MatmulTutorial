// =============================================================================
// FP8 GEMM Level 4 — SM100 (GB200) — 2-CTA Multicast on B
//
// D (BF16, M×N) = A (FP8 E4M3, M×K) × B^T (FP8 E4M3, N×K)
//
// Level 4 improvements over Level 3:
//   1. 2-CTA cluster: two CTAs cooperate per output tile pair
//   2. Each CTA loads half of B (LOAD_BLOCK_N = BLOCK_N/2 = 112)
//      → 50% bandwidth reduction for B
//   3. cta_group::2 MMA instruction processes both CTAs simultaneously
//      → UMMA_M = 256 (128 per CTA), more efficient compute
//   4. More pipeline stages (6 vs 4) due to smaller per-stage SMEM_B
//      → better latency hiding for TMA loads
//
// Architecture: cta_group::2 (2 CTAs in cluster)
//   - Two CTAs process DIFFERENT M-blocks but the SAME N-block
//   - CTA 0 loads B[:112,:], CTA 1 loads B[112:224,:]
//   - Leader CTA (rank 0) issues MMA for both CTAs
//   - Both CTAs run epilogue independently
//
// Warp assignment (256 threads = 8 warps per CTA):
//   Warp 0:   TMA Load (both CTAs: A full, B half, SFA own, SFB full)
//   Warp 1:   MMA Issue (leader CTA only; follower CTA idle)
//   Warp 2:   UTCCP Transposer (both CTAs: transpose SF, arrive at CTA 0)
//   Warp 3:   TMEM Allocator (both CTAs: cta_group::2 alloc, then idle)
//   Warp 4-7: Epilogue (both CTAs: read own TMEM, TMA store)
// =============================================================================
...
static constexpr uint32_t LOAD_BLOCK_N  = BLOCK_N / NUM_MULTICAST;  // 112

// MMA dimensions for cta_group::2
static constexpr uint32_t UMMA_M = BLOCK_M * NUM_MULTICAST;  // 256
static constexpr uint32_t UMMA_N = BLOCK_N;                  // 224
...
// SMEM sizes per pipeline stage (B uses LOAD_BLOCK_N, not BLOCK_N)
static constexpr uint32_t SMEM_A   = BLOCK_M * BLOCK_K * 1;
static constexpr uint32_t SMEM_B   = LOAD_BLOCK_N * BLOCK_K * 1;  // Half of Level 3
...
// TMEM layout (same column count as Level 3 — TMEM is per-CTA)
...
// Maximize pipeline stages (more stages with smaller SMEM_B)
...
static constexpr uint16_t CTA_MASK = (1 << NUM_MULTICAST) - 1;  // 0x3
...
// Uses mapa to map local SMEM address to CTA 0's equivalent address
...
// ----- UMMA (cta_group::2 for 2-CTA) -----

// MMA commit with multicast: signals barriers on BOTH CTAs simultaneously
...
// cta_group::2 MMA: processes 256×224 tile across 2 CTAs
...
// ----- UTCCP (cta_group::2 for 2-CTA) -----

// cta_group::2 UTCCP copy: reads SMEM from both CTAs, writes TMEM for both
...
// For cta_group::2: UMMA_M=256, m_dim = 256/16 = 16
// n_dim = BLOCK_N/8, scale_format = E8M0
__device__ __forceinline__ uint32_t make_fp8_idesc() {
    uint32_t d = 0;
    d |= ((UMMA_N / 8) << 17);    // n_dim
    d |= (1u << 23);              // scale_format = E8M0
    d |= ((UMMA_M / 16) << 24);   // m_dim (16 for 256, fits in 5-bit field)
    return d;
}
    ...
    uint32_t M, uint32_t N, uint32_t K)
{
    ...
    // Barrier pointers (each CTA has its own in its SMEM)
    ...
    if (wid == 1 && elect_one()) {
        for (uint32_t s = 0; s < CAPPED_STAGES; s++) {
            // full_bar & empty_bar: per-CTA, 1 arrival each
            bar_init(&full_bar[s], 1);
            bar_init(&empty_bar[s], 1);
            // sf_full_bar: 2*32=64 arrivals (both CTAs' transposer warps)
            bar_init(&sf_full_bar[s], NUM_MULTICAST * 32);
        }
        for (uint32_t e = 0; e < NUM_EPI_STAGES; e++) {
            // tf_bar: per-CTA, 1 arrival (multicast commit)
            bar_init(&tf_bar[e], 1);
            // te_bar: 2*128=256 arrivals (both CTAs' epilogue threads)
            bar_init(&te_bar[e], NUM_MULTICAST * N_EPI_THREADS);
        }
        bar_fence_init();
    }
    // TMEM allocation: cta_group::2 — both CTAs must call, synchronized
    if (wid == 3)
        tmem_alloc_2sm(tmem_smem, TMEM_COLS);

    // Cluster sync after init (replaces __syncthreads for multi-CTA)
    cluster_sync();
    ...
    //
    // Each CTA loads:
    //   - A: full BLOCK_M rows for its own M-block
    //   - B: LOAD_BLOCK_N cols (CTA 0→cols[0:112], CTA 1→cols[112:224])
    //   - SFA: for its own M-block
    //   - SFB: full BLOCK_N (both CTAs load the same SFB)
    // =====================================================================
    if (wid == 0 && elect_one()) {
        while (sched.next(mb, nb)) {
            for (uint32_t kb = 0; kb < num_kb; kb++) {
                ...
                // A: each CTA loads its own M-block
                tma_load(smem + O_A + stg * SMEM_A, &tma_a, &full_bar[stg],
                         (int32_t)k, (int32_t)(mb * BLOCK_M));

                // B: each CTA loads its half (offset by cta_rank * LOAD_BLOCK_N)
                ...
                if (kb % NUM_SF_PER_LOAD == 0) {
                    ...
                    // SFA: each CTA loads for its own M-block
                    tma_load(smem + O_SFA + stg * SMEM_SFA, &tma_sfa,
                             &full_bar[stg],
                             (int32_t)(mb * BLOCK_M), (int32_t)sfk);
                    // SFB: both CTAs load the full BLOCK_N
    ...
    //
    // Each CTA transposes its own SMEM data (SFA and SFB).
    // All threads arrive at CTA 0's sf_full_bar.
    // =====================================================================
    else if (wid == 2) {
        while (sched.next(mb, nb)) {
            for (uint32_t kb = 0; kb < num_kb; kb++) {
                // Wait for this CTA's own TMA load to complete
                ...
                // Arrive at CTA 0's sf_full_bar (cross-CTA arrival)
    ...
    //
    // Uses cta_group::2: reads A+B from both CTAs' SMEM,
    // writes accumulators to both CTAs' TMEM.
    // Issues multicast commits to signal both CTAs' barriers.
    // =====================================================================
    else if (wid == 1 && is_leader) {
        // SBO: stride byte offset for SMEM descriptors
        // A: 8 * BLOCK_K * sizeof(FP8) = 8 * 128 * 1 = 1024
        // B: 8 * BLOCK_K * sizeof(FP8) = 1024 (same, K-major, 128B swizzle)
        ...
        while (sched.next(mb, nb)) {
            ...
            // Wait for TMEM to be free (both CTAs' epilogues done)
            bar_wait(&te_bar[ai], ap ^ 1);
            tmem_fence_after();

            for (uint32_t kb = 0; kb < num_kb; kb++) {
                // Wait for BOTH CTAs' SF transposes (64 arrivals at CTA 0)
                bar_wait(&sf_full_bar[stg], ph);
                tmem_fence_after();

                // UTCCP copy SF to TMEM (cta_group::2: copies from both CTAs)
                ...
                // Multicast commit: signals BOTH CTAs' barriers
                if (elect_one()) {
                    if (kb == num_kb - 1)
    ...
    //
    // Each CTA reads its own TMEM, writes to SMEM, TMA stores to its
    // own output location. All epilogue threads arrive at CTA 0's te_bar.
    // =====================================================================
    else if (wid >= 4) {
        uint32_t ew = wid - 4;
        ...
        while (sched.next(mb, nb)) {
            ...
            // Wait for MMA to complete (local barrier, signaled by multicast)
            ...
            for (uint32_t s = 0; s < CD_STORES; s++) {
                ...
                if (s == CD_STORES - 1) {
                    tmem_fence_before();
                    // Arrive at CTA 0's te_bar (cross-CTA arrival)
                ...
                if (ew == 0 && elect_one()) {
                    // Each CTA stores to its own M-block
        ...
        tma_store_wait<0>();
        // TMEM dealloc: cta_group::2, both CTAs must call
        if (ew == 3)
    ...
    int M, int N, int K)
{
    ...
    // B: FP8 K-major — box size uses LOAD_BLOCK_N (half of BLOCK_N)
    CUtensorMap tma_b;
    {
        uint64_t d[2] = {(uint64_t)K, (uint64_t)N};
        uint64_t s[1] = {(uint64_t)K};
        uint32_t b[2] = {BLOCK_K, LOAD_BLOCK_N};  // Half-width box
        uint32_t e[2] = {1, 1};
    ...
    // SFB: int32 — both CTAs load full BLOCK_N
    CUtensorMap tma_sfb;
    {
    ...
    // Launch config: grid must be multiple of cluster size (2)
    int num_sms;
    ...
    // Round down to multiple of cluster size
    ...
    attrs[0].val.clusterDim = {NUM_MULTICAST, 1, 1};  // 2-CTA cluster
    config.attrs    = attrs;
    config.numAttrs = 1;
