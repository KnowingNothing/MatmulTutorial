...
def linear_attention_bwd_kernel(
    ...
    for m in range(start_m * BLOCK_M, seq_len, BLOCK_M):          
        ...
        iq = tl.load(iq_ptrs, boundary_check=(0, 1))
        rk = tl.load(n_rk_ptrs, boundary_check=(0, 1))
        ik = tl.load(n_ik_ptrs, boundary_check=(0, 1))
        rk_T = tl.trans(rk)
        ik_T = tl.trans(ik)
        ...
        # compute gv
        rgo = tl.load(rgo_ptrs, boundary_check=(0, 1))
        igo = tl.load(igo_ptrs, boundary_check=(0, 1))
        ...
        tl.atomic_add(rgq_ptrs, rgq)
        tl.atomic_add(igq_ptrs, igq)
        ...
        rgq_ptrs += BLOCK_M * stride_qm
        igq_ptrs += BLOCK_M * stride_qm
    ...
    tl.store(n_rgv_ptrs, rgv, boundary_check=(0, 1))
    tl.store(n_igv_ptrs, igv, boundary_check=(0, 1))
    
    rgk = (gpq_rr.to(DTYPE) - gpq_ii.to(DTYPE))
    igk = (gpq_ri.to(DTYPE) + gpq_ir.to(DTYPE))
    tl.store(n_rgk_ptrs, rgk, boundary_check=(0, 1))
    tl.store(n_igk_ptrs, igk, boundary_check=(0, 1))
